...
function SignIn() {
  const { user, handleSignIn: signIn, handleSignUpWithPassword: signUp } = useAuth();
  ...
  return (
    <Container sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', backgroundColor: "#f5f5f5", flexDirection: 'column', gap: 2, borderRadius: 7 }}>
      <Paper sx={{ padding: 5, width: { xs: 350, sm: 400 } }}>
        <Typography textAlign={"center"} sx={{ fontWeight: "500", fontSize: 30, marginBottom: 3 }}>Sign Up</Typography>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 2, margin: 3 }}>
          <Button variant={isLoading ? "contained" : "outlined"} color="primary" fullWidth disabled={isLoading} onClick={handleSubmit(onSignUp)}>
            Sign Up
          </Button>
        </Box>
